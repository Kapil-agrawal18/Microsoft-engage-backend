...
from rest_framework.response import Response
from .emailer import emailRecords
...
class CriminalImgDetectViews(APIView):
    ...
    def post(self, request):
        ...
        test_img_url = r"C:\Users\tanma\Desktop\Microsoft\rest-api2" + test_img_url
        ...
        var = r"C:\Users\tanma\Desktop\Microsoft\rest-api2\media\images_detected\image_" + str(userid) + ".jpg"
        ...
        return Response({"status": "Criminal Detected", "Criminals_Detected": result, "Processed_Image": return_url}, status=status.HTTP_200_OK)
...
class CriminalVideoDetectViews(APIView):
    ...
    def post(self, request):
        ...
        test_video_url = r"C:\Users\tanma\Desktop\Microsoft\rest-api2" + test_video_url
        ...
        userid = request.user.id
        var = r"C:\Users\tanma\Desktop\Microsoft\rest-api2\media\videos_detected\video_" + str(userid) + ".mp4"
        ...
        var_csv = r"C:\Users\tanma\Desktop\Microsoft\rest-api2\media\criminal_records\video_timestamps_" + str(userid) + ".csv"
        ...
        return Response({"status": "Criminals Detected", "Criminals_Detected": result, "Processed_Video": return_url}, status=status.HTTP_200_OK)
...
class loginUser(APIView):
    def post(self, request):
        ...
        if password == "":
            if image is None:
                print("IMAGE NONE")
                return Response({'msg': 'Invalid cridential'})
            else:
                print("IMAGE YES")
                suspect = CriminalImgDetect.objects.create(query_image_url = image, name = "suspect_name")
                suspect.save()
                temp = CriminalImgDetect.objects.get(name = "suspect_name")          
                img_url = temp.query_image_url.url
                img_url = img_url.replace("/", "\\")
                img_url = r"C:\Users\tanma\Desktop\Microsoft\rest-api2" + img_url
                ...
                user_encoding = eval(profile.encoding[7:-2])
                if(face_recognition.compare_faces([user_encoding], img_encoding)[0]):
                    k = requests.post('http://localhost:8000/api/token/', data={'username': username,'password': profile.password})
                    k = json.loads(k.content)
                    print(k)
                    return Response({'access': k['access'], 'refresh': k['refresh']})
                else:
                    return Response(
                        {
                            "msg": "Images not matching"
                        })           
            ...
            k = requests.post('http://localhost:8000/api/token/', data={'username': username,'password': password})
            k = json.loads(k.content)
            return Response({'access': k['access'], 'refresh': k['refresh']})
